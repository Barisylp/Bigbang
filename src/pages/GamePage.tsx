...
const GamePage: React.FC<GamePageProps> = ({ currentRoom, socket }) => {
    ...
    const [lastDiceResult, setLastDiceResult] = React.useState<number | null>(null);
    const [diceWinner, setDiceWinner] = React.useState<string | null>(null);
    ...
    const [chatTarget, setChatTarget] = React.useState('everyone');
    ...
    React.useEffect(() => {
        ...
        socket.on('diceRolled', ({ playerName, result }: { playerName: string, result: number }) => {
            setDiceRolling(true);
            setDiceWinner(playerName);
            setLastDiceResult(null);

            // Dice rolling animation feel
            setTimeout(() => {
    ...
    return (
        <div className="min-h-screen bg-slate-900 text-white flex flex-col overflow-hidden">
            ...
            {myPlayer && myPlayer.gender === null && currentRoom.started && (
                <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/80 backdrop-blur-md">
                    <div className="bg-slate-900 p-8 rounded-3xl border-4 border-amber-500 shadow-[0_0_50px_rgba(245,158,11,0.3)] max-w-md w-full text-center flex flex-col gap-6">
                        <h2 className="text-3xl font-black text-white italic">CİNSİYETİNİ SEÇ!</h2>
                        <p className="text-slate-400 text-sm">Bu seçim oyun sonuna kadar değiştirilemez ve tüm oyuncular tarafından görülür.</p>
                        <div className="flex justify-around gap-4 mt-4">
                            <button
                                onClick={() => socket.emit('setGender', { roomId: currentRoom.id, gender: 'male' })}
                                className="flex-1 flex flex-col items-center gap-2 bg-blue-900/40 hover:bg-blue-800/60 p-6 rounded-2xl border-2 border-blue-500/50 transition-all transform hover:scale-105"
                            >
                                <span className="text-6xl">♂</span>
                                <span className="font-bold text-blue-100">ERKEK</span>
                            </button>
                            <button
                                onClick={() => socket.emit('setGender', { roomId: currentRoom.id, gender: 'female' })}
                                className="flex-1 flex flex-col items-center gap-2 bg-pink-900/40 hover:bg-pink-800/60 p-6 rounded-2xl border-2 border-pink-500/50 transition-all transform hover:scale-105"
                            >
                                <span className="text-6xl">♀</span>
                                <span className="font-bold text-pink-100">KADIN</span>
                            </button>
                        </div>
                    </div>
                </div>
            ...
            {(diceRolling || lastDiceResult !== null) && (
                <div className="fixed top-20 left-1/2 transform -translate-x-1/2 z-[1000] flex flex-col items-center gap-2">
                    <div className="bg-slate-900/90 px-6 py-2 rounded-full border border-slate-700 shadow-2xl flex items-center gap-3">
                        <span className="text-amber-500 font-bold">{diceWinner}</span>
                        <span className="text-white text-sm">Zar Atıyor...</span>
                        <div className={`w-12 h-12 flex items-center justify-center bg-white rounded-lg shadow-inner text-2xl font-black text-slate-900 ${diceRolling ? 'animate-spin' : 'animate-bounce'}`}>
            ...
            <div className="flex-1 p-4 flex flex-col gap-6 overflow-y-auto custom-scrollbar relative">
                ...
                <div className="flex flex-wrap justify-center gap-4 bg-slate-900/40 p-4 rounded-2xl border border-slate-700/30 backdrop-blur-sm w-full shrink-0">
                    {currentRoom.players.filter((p: any) => p.id !== socket.id).map((p: any) => (
                        <div key={p.id} className="bg-slate-800/80 p-3 rounded-xl border border-slate-600/50 flex flex-col items-center min-w-[200px] shadow-lg">
                            <div className="font-bold text-sm mb-1 flex items-center gap-2">
                                {p.name}
                                {p.gender === 'male' && <span className="text-blue-400">♂</span>}
                                {p.gender === 'female' && <span className="text-pink-400">♀</span>}
                            </div>
                            <div className="flex items-center gap-2 mb-2 text-[10px]">
                                ...
                                <span className="text-slate-400">🃏 {p.hand.length}</span>
                            </div>

                            <div className="mb-2 w-full scale-90 origin-top">
                                <PlayerInventory player={p} />
                            </div>

                            {/* OTHER PLAYER DEBUFFS */}
                            {p.activeModifiers && p.activeModifiers.length > 0 && (
                                <div className="w-full bg-red-900/20 p-1 rounded border border-red-800/50 flex flex-col mb-1 overflow-hidden">
                                    {p.activeModifiers.map((mod: any, idx: number) => (
                                        <div key={`op-mod-${idx}`} className="flex justify-between items-center text-[8px] text-red-200 bg-red-950/50 px-1 rounded mb-0.5">
                            ...
                            <div className="flex justify-center gap-0.5 mt-1">
                                {p.hand.map((_: any, i: number) => (
                                    <div key={i} className="w-1.5 h-2.5 bg-slate-600 rounded-sm border border-slate-500"></div>
                                ))}
                            </div>
                        </div>
                    ))}
                    {currentRoom.players.length === 1 && (
                        <div className="text-slate-500 text-xs italic py-4">Diğer oyuncular bekleniyor...</div>
                    )}
                </div>

                {/* CENTRAL GAME TABLE AREA (Now stacks naturally) */}
                <div className="flex flex-1 flex-col items-center justify-center min-h-[400px] relative z-10">
                                    ...
                                    {isMe ? (
                                        <div className="flex flex-col items-center gap-4 w-full">
                                            <div className="flex gap-4 w-full max-w-md">
                                                <button
                                                    onClick={() => socket.emit('rollDiceToEscape', { roomId: currentRoom.id })}
                                                    disabled={diceRolling || currentRoom.currentCombat?.isResolving}
                                                    className="flex-1 bg-gradient-to-r from-amber-600 to-amber-700 text-white font-black py-4 rounded-xl shadow-xl hover:from-amber-500 hover:to-amber-600 transition-all transform hover:scale-105 border-b-8 border-amber-900 disabled:opacity-50"
                                                >
                                                    🎲 ZAR AT (5-6 Kaçar)
                                                </button>
                                                <button
                                                    onClick={() => socket.emit('resolveCombat', { roomId: currentRoom.id })}
                                                    className={`flex-1 text-white font-black py-4 rounded-xl shadow-xl transition-all hover:scale-105 active:scale-95 border-b-8
                                                        ${(combatantPower + (currentRoom.currentCombat.playerBonus || 0)) > ((currentRoom.currentCombat.monster?.level || 0) + (currentRoom.currentCombat.monsterBonus || 0))
                                                            ? 'bg-gradient-to-r from-green-600 to-green-500 border-green-800 hover:from-green-500 hover:to-green-400'
                                                            : 'bg-gradient-to-r from-red-800 to-red-600 border-red-950 hover:from-red-700 hover:to-red-500'}`}
                                                >
                                                    {(combatantPower + (currentRoom.currentCombat.playerBonus || 0)) > ((currentRoom.currentCombat.monster?.level || 0) + (currentRoom.currentCombat.monsterBonus || 0)) ? "SAVAŞI ÇÖZ" : "PES ET"}
                                                </button>
                                            </div>
                                            <p className="text-slate-500 text-xs italic">Kaybetmek yerine zar atarak kaçmayı deneyebilirsin.</p>
                                        </div>
                                    ) : (
                ...
                {myTurn && currentRoom.turnPhase === 'action_selection' && (
                    <div className="mt-8 flex flex-col items-center gap-2 z-20 bg-slate-900/90 p-4 rounded-xl border border-amber-500/50 shadow-2xl backdrop-blur-sm">
            ...
            <div className="bg-slate-800 p-3 border-t border-slate-700 shrink-0">
                ...
                <div className="flex justify-between items-start mb-2">
                    <div>
                        <h2 className="text-2xl font-bold flex items-center gap-2">
                            {myPlayer?.name}
                            {myPlayer?.gender === 'male' && <span className="text-blue-400 text-xl font-normal">♂</span>}
                            {myPlayer?.gender === 'female' && <span className="text-pink-400 text-xl font-normal">♀</span>}
                ...
                <div className="flex gap-4 items-end">
                    {/* LEFT: MY HAND */}
                    <div className="flex-1 overflow-x-auto pb-2 px-1 custom-scrollbar">
                    ...
                    <div className="w-80 flex flex-col gap-2 shrink-0">
                            ...
                            <div className="flex flex-col gap-2 items-stretch">
                                ...
                                {currentRoom.currentCombat?.status === 'active' && currentRoom.currentCombat.playerId === socket.id && !currentRoom.currentCombat.allyId && (
                                    <button
                                        ...
                                        className="bg-amber-500 hover:bg-amber-400 text-black font-black py-2 px-6 rounded-lg shadow-lg border-b-4 border-amber-700 active:border-b-0 active:translate-y-1 transition-all w-full"
                                    >
                                        🤝 MÜTTEFİK ARA
                                    </button>
                                )}
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <button
                                        ...
                                        className={`
                                            flex-1 flex flex-col items-center justify-center px-4 py-1 rounded-lg border-b-4 transition-all
                                        ...
                                        <button
                                            onClick={() => {
                                                ...
                                                if (myPlayer.hand.length > 5) {
                                                    alert(`Elinizde ${myPlayer.hand.length} kart var! Turu bitirmek için en fazla 5 kartınız olmalı.`);
                                                    return;
                                                }
                                                socket.emit('endTurn', currentRoom.id);
                                            }}
                                            disabled={currentRoom.turnPhase === 'kick_open'}
                                            className={`flex-1 ${myPlayer.hand.length > 5 ? 'bg-red-600 hover:bg-red-500 animate-pulse' : 'bg-blue-600 hover:bg-blue-500'} text-white font-bold py-2 px-4 rounded-lg shadow-lg border-b-4 ${myPlayer.hand.length > 5 ? 'border-red-800' : 'border-blue-800'} active:border-b-0 active:translate-y-1 transition-all
                                            ${currentRoom.turnPhase === 'kick_open' ? 'opacity-50 cursor-not-allowed' : ''}`}
                                        >
                                            {myPlayer.hand.length > 5 ? `At (${myPlayer.hand.length}/5)` : 'BİTİR'}
                                        </button>
                                    )}
                                </div>
                            </div>
                        ...
                        {/* CHAT BOX */}
                        <div className="flex flex-col bg-slate-900/80 rounded-xl border border-slate-700 overflow-hidden shadow-inner">
                            <div className="bg-slate-800 px-3 py-1 border-b border-slate-700 flex justify-between items-center">
                                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Sohbet</span>
                                <div className="flex gap-1">
                                    <span className={`w-2 h-2 rounded-full ${chatTarget === 'everyone' ? 'bg-green-500' : 'bg-amber-500'} animate-pulse`}></span>
                                    <span className="text-[9px] text-slate-500">{chatTarget === 'everyone' ? 'Herkes' : 'Özel'}</span>
                                </div>
                            </div>

                            <div
                                ref={chatScrollRef}
                                className="h-40 overflow-y-auto p-2 flex flex-col gap-1 text-xs custom-scrollbar"
                            >
                                {currentRoom.messages?.map((msg: any) => (
                                    <div key={msg.id} className={`flex flex-col ${msg.senderId === socket.id ? 'items-end' : 'items-start'}`}>
                                        <div className={`p-2 rounded-lg max-w-[90%] break-words 
                                            ${msg.targetId
                                        ...
                                        >
                                            <div className="flex items-center gap-1 mb-0.5">
                                                <span className="text-[9px] font-bold opacity-60">
                                                    {msg.senderId === socket.id ? 'Sen' : msg.senderName}
                                                </span>
                                                {msg.targetId && <span className="text-[8px] bg-amber-800 px-1 rounded text-amber-300">ÖZEL</span>}
                                            </div>
                                            <div>{msg.text}</div>
                                        </div>
                                    </div>
                                ...
                                {(!currentRoom.messages || currentRoom.messages.length === 0) && (
                                    <div className="text-center text-slate-600 italic text-[10px] mt-4">Henüz mesaj yok...</div>
                                )}
                            </div>

                            <div className="p-2 bg-slate-800/50 flex flex-col gap-1 border-t border-slate-700">
                                <div className="flex gap-1">
                                    <select
                                        value={chatTarget}
                                        onChange={(e) => setChatTarget(e.target.value)}
                                        className="flex-1 bg-slate-900 text-[10px] rounded px-1 py-0.5 border border-slate-700 text-slate-300 outline-none focus:border-amber-500"
                                    >
                                        ...
                                        onSubmit={(e) => {
                                            ...
                                            if (chatMessage.trim()) {
                                                socket.emit('sendMessage', {
                                                    roomId: currentRoom.id,
                                                    text: chatMessage,
                                                    targetId: chatTarget === 'everyone' ? undefined : chatTarget
                                                });
                                                setChatMessage('');
                                            }
                                        }}
                                    >
                                        <input
                                            ...
                                            placeholder="..."
                                            className="flex-1 bg-slate-900 border border-slate-700 rounded px-2 py-0.5 text-xs text-white outline-none focus:border-blue-500"
                                        />
                                        <button
                                            type="submit"
                                            className="bg-blue-600 hover:bg-blue-500 text-white px-2 py-0.5 rounded text-[10px] font-bold transition-colors"
                                        >
                                            OK
                                        </button>
                                    </form>
